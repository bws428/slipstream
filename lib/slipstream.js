...
$(document).ready(() => {
  ...
  const header = [
    ...
    "PIC Name",
    "SIC Name"
    // 'Passenger List', // TODO: Can we include FA and DH names somehow?
  ];
  ...
  (async () => {
    ...
    const table = buildTable(flights, pairing);
 ...
 * @param {string} pairing - The pairing number.
 */
function downloadCsv(csv, pairing) {
 ...
 */
function buildTable(flights, pairing) {
  const table = [];
